...
def get_args():
    ...
    parser.add_argument("--image", "-i", type=str, required=False, default="model_convert/inpaint_samples/inpaint.png", help="the inpaint image")
    parser.add_argument("--mask", "-m", type=str, required=False, default="model_convert/inpaint_samples/inpaint_mask.png", help="the inpaint mask")
    ...
    parser.add_argument("--unet_model", "-u", type=str, required=False, default="./models/inpaint/unet.axmodel", help="the dir of unet.axmodel")
    parser.add_argument("--vae_encoder_model", "-v", type=str, required=False, default="./models/inpaint/vae_encoder.axmodel", help="the dir of vae_encoder.axmodel")
    parser.add_argument("--vae_decoder_model", "-v", type=str, required=False, default="./models/inpaint/vae_decoder.axmodel", help="the dir of vae_decoder.axmodel")
...
if __name__ == '__main__':
    ...
    vae_encoder = InferenceSession.load_from_model(vae_encoder_model)
    ...
    encoder_output = vae_encoder.run({"x": masked_image.detach().numpy()})['683']
    masked_image_latents = DiagonalGaussianDistribution(encoder_output).sample(None)
    ...
    vae_decoder = InferenceSession.load_from_model(vae_decoder_model)
    ...
    for i, timestep in enumerate(timesteps):
        ...
        noise_pred = unet_session_main.run({"sample": latent_model_input.detach().numpy(), \
    ...
    image = vae_decoder.run({"x": latent.detach().numpy()})['784']
